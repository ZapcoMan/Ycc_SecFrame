...
class DudeSuiteApp(QMainWindow):
    ...
    def create_software_description_page(self):
        ...
        # 信息卡片
        info_card = QFrame()
        info_card.setStyleSheet('background-color: #2a2a2a; border-radius: 10px; padding: 15px;')
        ...
        # 软件名称
        name_label = QLabel('Ycc_SecFrame 框架')
        name_label.setStyleSheet('font-size: 18px; font-weight: bold; color: #4da6ff;')
        text_layout.addWidget(name_label)

        # 作者
        author_label = QLabel('作者: 杨CC')
        author_label.setStyleSheet('font-size: 14px; color: #ddd;')
        text_layout.addWidget(author_label)

        # 开源地址
        github_label = QLabel('<a href="https://github.com/Sgyling/Ycc_SecFrame" style="color: #4da6ff; text-decoration: none;">开源地址: https://github.com/Sgyling/Ycc_SecFrame</a>')
        github_label.setStyleSheet('font-size: 14px; color: #ddd;')
        github_label.setTextFormat(Qt.RichText)
        github_label.setTextInteractionFlags(Qt.TextSelectableByMouse | Qt.TextBrowserInteraction)
        github_label.setOpenExternalLinks(True)
        text_layout.addWidget(github_label)

        # 官方网站
        website_label = QLabel('<a href="https://Yancy77.cn" style="color: #4da6ff; text-decoration: none;">官方网站: https://Yancy77.cn</a>')
        website_label.setStyleSheet('font-size: 14px; color: #ddd;')
        website_label.setTextFormat(Qt.RichText)
        website_label.setTextInteractionFlags(Qt.TextSelectableByMouse | Qt.TextBrowserInteraction)
        website_label.setOpenExternalLinks(True)
        text_layout.addWidget(website_label)

        info_card_layout.addWidget(text_container, 1)
        content_layout.addWidget(info_card)

        # 给作者点一杯咖啡
        section_label = QLabel('### 支持作者')
        section_label.setStyleSheet('font-size: 18px; font-weight: bold; color: #4da6ff; margin-top: 15px;')
        content_layout.addWidget(section_label)

        donate_label = QLabel('如果觉得这个工具对你有帮助，可以请作者喝杯咖啡:')
        donate_label.setStyleSheet('font-size: 14px; color: #ddd; margin-bottom: 10px;')
        ...
        if os.path.exists(wx_path):
            wx_label = QLabel()
            wx_pixmap = QPixmap(wx_path)
            wx_pixmap = wx_pixmap.scaled(300, 300, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            wx_label.setPixmap(wx_pixmap)
            wx_layout.addWidget(wx_label)
        wx_text = QLabel('微信支付')
        wx_text.setStyleSheet('font-size: 12px; color: #ddd;')
        wx_text.setAlignment(Qt.AlignCenter)
        wx_layout.addWidget(wx_text)
        ...
        if os.path.exists(zfb_path):
            zfb_label = QLabel()
            zfb_pixmap = QPixmap(zfb_path)
            zfb_pixmap = zfb_pixmap.scaled(300, 300, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            zfb_label.setPixmap(zfb_pixmap)
            zfb_layout.addWidget(zfb_label)
        zfb_text = QLabel('支付宝')
        zfb_text.setStyleSheet('font-size: 12px; color: #ddd;')
        zfb_text.setAlignment(Qt.AlignCenter)
        zfb_layout.addWidget(zfb_text)
        qr_layout.addLayout(zfb_layout)
    ...
    def create_home_page(self):
        ...
        # 添加版本信息
        version_label = QLabel('Version 1.1.0.0 - 杨CC')
